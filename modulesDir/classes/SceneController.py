...
class SceneController:
	...
	def selectAdjuscentFacesAbove(self,objectName,compareAxis,keepOrigFaceSelected):
		object = bpy.data.objects[objectName]
		mesh = object.data
		...
		for face in selectedFaces:
			for edge in face.edges:
				linkedFaces = edge.link_faces
				for linkedFace in linkedFaces:
					if self.localToGlobalCoords(face.calc_center_median())[compareAxis]<self.localToGlobalCoords(linkedFace.calc_center_median())[compareAxis]:
						linkedFace.select = True
		if not keepOrigFaceSelected:
			face.select=False
		bmesh.update_edit_mesh(mesh, True)

	def selectAdjuscentFacesBelow(self,objectName,compareAxis,keepOrigFaceSelected):
		object = bpy.data.objects[objectName]
		mesh = object.data
	...
	def getNeighbFacesNum(self,objName):
		object = bpy.data.objects[objName]
		mesh = object.data
		counter=0
		bm = bmesh.from_edit_mesh(mesh)
	...
	def getSelectedNeighbFacesNum(self,objName):
		object = bpy.data.objects[objName]
		mesh = object.data
		counter=0
		bm = bmesh.from_edit_mesh(mesh)
		...
		for face in selectedFaces:
			for edge in face.edges:
				linkedFaces = edge.link_faces
				for linkedFace in linkedFaces:
					if linkedFace.select:
						counter+=1
		return counter

	#get perimeter selected faces indexes out of currently selected faces
	def getPerimeterSelectedFacesIndexes(self,objName):
		object = bpy.data.objects[objName]
		mesh = object.data
		counter=0
		bm = bmesh.from_edit_mesh(mesh)
		...
		for faceInd in range(0,len(faces)):
			...
			if not face.select:
				continue
			for edgeInd in range(0,len(face.edges)):
				linkedFaces =[linkedFace for linkedFace in face.edges[edgeInd].link_faces if linkedFace.select]
				if len(linkedFaces)==1:
					result+=[faceInd] #if the current face has at least 1 edge with no linked faces, this is a perimeter face
					break
		return result
